"""LangGraph agent using WTA trails, Mapbox geocoding, and weather via MCP servers."""
...
# Project root - for stdio server cwd (run from project root)
_PROJECT_ROOT = str(Path(__file__).resolve().parent.parent.parent.parent)

# MCP server config - wta-trails via URL (must run: python3 -m beta_graph.servers.wta.server --http)
# mapbox and weather via stdio (agent spawns them)
MCP_SERVERS = {
    "wta-trails": {
        "transport": "sse",
...
SYSTEM_PROMPT = """You are a helpful hiking assistant that uses WTA (Washington Trails Association) trails, Mapbox geocoding, and weather forecasts.
...
- search_trails returns trails with: Parking/Pass, Alerts, Getting there, Features, Conditions (trip reports).
...
def run_cli():
    """CLI entry for WTA agent. Uses MCP servers (wta-trails, mapbox, weather)."""
    import sys
    ...
    async def _run():
        ...
        print("WTA + Mapbox + Weather agent (MCP). Type 'quit' or 'exit' to stop.\n")
        print("Ensure WTA server is running: python3 -m beta_graph.servers.wta.server --http\n")
    ...
    try:
        ...
        print(f"Error: {e}")
        if "Connection refused" in str(e) or "8001" in str(e):
            print("\nMake sure WTA server is running: python3 -m beta_graph.servers.wta.server --http")
        raise
